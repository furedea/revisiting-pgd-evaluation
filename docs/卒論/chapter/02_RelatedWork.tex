...
$t$回目の反復における点を$\bm{x}_t$，モデルのロジット（softmax適用前の出力）を$Z(\bm{x}) = (Z_1(\bm{x}), \ldots, Z_K(\bm{x}))$とする．ここで$K$はラベル数である．現在の予測ラベルを
\begin{align}
...
\subsection{最短摂動の導出}
線形近似されたモデルにおいて，ラベル$k$の境界（$g_k(\bm{x}) = 0$）までの$\ell_2$最短摂動は以下のように導出される：
\begin{align}
