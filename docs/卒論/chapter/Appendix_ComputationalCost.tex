...
以上の分析から，提案手法は追加の計算コストを要することが明らかになった．ただし，本分析は最悪ケースを想定しているため，実際の計算コストはこれより低くなる場合がある．DeepFool初期化は計算コストの削減を目的とした手法ではなく，初回誤分類反復数の削減効果を検証するための手法として位置づけられる．Multi-DeepFool初期化については，誤分類達成までのPGD反復数を50回未満に削減できる場合に限り計算効率の改善が見込まれる．
