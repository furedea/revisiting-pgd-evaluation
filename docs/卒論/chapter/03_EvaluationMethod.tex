...
PGD攻撃の目的は，モデルに誤分類を引き起こすことである．本節では，攻撃成功（誤分類）の判定基準と，評価に用いる「誤分類到達反復数」の定義を与える．

\subsection{誤分類到達反復数の定義}
\label{sec:first_misclassification_iteration}

PGD攻撃の効果を定量的に評価するため，\textbf{誤分類到達反復数}を導入する．これは，モデルの予測が正解ラベルから不正解ラベルに変わった最初の反復数である．

$t$回目の反復における敵対的サンプルを$x^t$，正解ラベルを$y$，敵対的サンプルに対するモデルの予測ラベルを$k(x)$とすると，誤分類到達反復数$t^*$は式(\ref{eq:first_misclassification})で定義する．
\begin{align}
    t^* = \min\{t : k(x^t) \neq y\}
...
\subsection{誤分類到達反復数の統計量}
\label{sec:misclassification_statistics}

攻撃成功率は全体的な成功率を示すが，誤分類に要した反復数の分布については情報を与えない．そこで，誤分類到達反復数の分布を特徴づけるため，以下の統計量を計算する：
\begin{itemize}
    \item \textbf{平均}：誤分類到達反復数の算術平均．全体の目安となる．
    \item \textbf{中央値（Median）}：誤分類到達反復数の中央値．外れ値の影響を受けにくい．
    \item \textbf{95パーセンタイル（P95）}：誤分類到達反復数の分布において95\%の試行が誤分類に到達する反復数．最悪ケースに近い．
\end{itemize}

これらの統計量を組み合わせることで，誤分類到達反復数の分布を多面的に把握できる．例えば，平均と中央値が大きく異なる場合は，一部の試行で誤分類が著しく遅いことを示唆する．P95は，ほぼ全ての試行が誤分類に到達するために必要な反復数の実用的な目安として重要である．
