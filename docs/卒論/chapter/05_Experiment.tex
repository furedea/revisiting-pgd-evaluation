...
表\ref{table:models}に使用するモデルの詳細を示す．natおよびadvモデルは，Madryらが公開しているリポジトリ\cite{MadryMNIST, MadryCIFAR10}から入手した事前学習済みモデルである．ただし，advモデルはPGD攻撃に対して極めて頑強であり，100反復以内に誤分類が達成されなかったため，初回誤分類反復数の評価が困難であった．そこで，natとadvの中間的なロバスト性を持つと期待されるnat\_and\_advおよびweak\_advモデルを，同リポジトリのtrain.pyを修正して追加で学習させた．
...
これらのモデル間での誤分類性能の違いを比較することで，モデルのロバスト性と初回誤分類反復数の関係を調査する．
...
この選択基準は，PGD攻撃の前提条件である「元々正しく分類されているサンプルに対する攻撃」を満たすためである．攻撃前から誤分類されているサンプルを評価対象に含めると，攻撃の効果を正しく評価できない．また，全てのモデルで共通のサンプルを使用することで，異なるモデル間での損失曲線や初回誤分類反復数の比較が容易になる．
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{初回誤分類反復数の定量分析}
\label{sec:misclassification_analysis}

本節では，第\ref{sec:misclassification_definition}節で定義した初回誤分類反復数を用いて，PGD攻撃の誤分類性能を定量的に分析する．全4モデル（nat, adv, nat\_and\_adv, weak\_adv）および全4初期化手法（クリーン，ランダム，DeepFool，Multi-DeepFool）の結果を比較する．
...
\begin{figure}[H]
  ...
  \caption{MNISTにおける誤分類CDF．横軸は反復数，縦軸はその反復数までに誤分類を達成したサンプルの累積割合．色は初期化手法を表す（青：クリーン，緑：ランダム，赤：DeepFool，オレンジ：Multi-DeepFool）．マーカーと線種はモデルを表す（丸・実線：nat，四角・破線：nat\_and\_adv，三角・一点鎖線：adv，ダイヤ・点線：weak\_adv）．攻撃に失敗した組み合わせはグレーで表示され，凡例に[FAILED]と記載．}
  \label{fig:mnist_misclassification_cdf}
\end{figure}

\paragraph{誤分類CDFの読み方}
誤分類CDFは，横軸に反復数，縦軸にその反復数までに誤分類を達成したサンプルの累積割合を示す．各曲線は初期化手法とモデルの組み合わせに対応している．曲線が左側にあるほど少ない反復数で誤分類を達成しており，初回誤分類反復数が少ないことを意味する．曲線が縦軸の1.0に到達しない場合は，100反復以内に誤分類を達成できなかったサンプル（攻撃失敗）が存在することを示す．
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|c|cccc}
    ...
    \hline
    \multirow{4}{*}{nat} & clean & 1 & 100\% & 12.0 & 12.0 & 12.0 \\
    & random & 20 & 100\% & 6.6 & 7.0 & 8.0 \\
    & deepfool & 1 & 100\% & 0.0 & 0.0 & 0.0 \\
    & multi\_deepfool & 9 & 100\% & 0.7 & 1.0 & 1.0 \\
    \hline
    \multirow{4}{*}{nat\_and\_adv} & clean & 1 & 0\% & --- & --- & --- \\
    & random & 20 & 0\% & --- & --- & --- \\
    & deepfool & 1 & 0\% & --- & --- & --- \\
    & multi\_deepfool & 9 & 0\% & --- & --- & --- \\
    \hline
    ...
    & multi\_deepfool & 9 & 0\% & --- & --- & --- \\
    \hline
    \multirow{4}{*}{weak\_adv} & clean & 1 & 100\% & 35.0 & 35.0 & 35.0 \\
    & random & 20 & 100\% & 25.5 & 25.0 & 29.1 \\
    & deepfool & 1 & 100\% & 22.0 & 22.0 & 22.0 \\
    & multi\_deepfool & 9 & 100\% & 28.1 & 28.0 & 33.0 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{CIFAR10における誤分類CDF．横軸は反復数，縦軸はその反復数までに誤分類を達成したサンプルの累積割合．色は初期化手法を表す（青：クリーン，緑：ランダム，赤：DeepFool，オレンジ：Multi-DeepFool）．マーカーと線種はモデルを表す（丸・実線：nat，四角・長破線：nat\_and\_adv，三角・一点鎖線：adv，ダイヤ・点線：weak\_adv）．攻撃に失敗した組み合わせはグレーで表示され，凡例に[FAILED]と記載．同じ反復数で誤分類を達成した曲線が重なるため，視認性のために横方向に微小なオフセットを加えている．}
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|c|cccc}
    ...
    モデル & 初期化 & リスタート数 & 攻撃成功率 & 平均 & 中央値 & P95 \\
    \hline
    \multirow{4}{*}{nat} & clean & 1 & 100\% & 1.0 & 1.0 & 1.0 \\
    & random & 20 & 100\% & 1.3 & 1.0 & 2.0 \\
    & deepfool & 1 & 100\% & 0.0 & 0.0 & 0.0 \\
    & multi\_deepfool & 9 & 100\% & 0.3 & 0.0 & 1.0 \\
    \hline
    \multirow{4}{*}{nat\_and\_adv} & clean & 1 & 100\% & 1.0 & 1.0 & 1.0 \\
    & random & 20 & 100\% & 1.0 & 1.0 & 1.0 \\
    & deepfool & 1 & 100\% & 0.0 & 0.0 & 0.0 \\
    & multi\_deepfool & 9 & 100\% & 0.0 & 0.0 & 0.0 \\
    \hline
    \multirow{4}{*}{adv} & clean & 1 & 0\% & --- & --- & --- \\
    & random & 20 & 0\% & --- & --- & --- \\
    & deepfool & 1 & 0\% & --- & --- & --- \\
    & multi\_deepfool & 9 & 0\% & --- & --- & --- \\
    \hline
    \multirow{4}{*}{weak\_adv} & clean & 1 & 100\% & 2.0 & 2.0 & 2.0 \\
    & random & 20 & 100\% & 2.0 & 2.0 & 2.0 \\
    & deepfool & 1 & 100\% & 1.0 & 1.0 & 1.0 \\
    & multi\_deepfool & 9 & 100\% & 0.1 & 0.0 & 0.6 \\
    \hline
  \end{tabular}
\end{table}
...
本節では，損失曲線の観察（第\ref{sec:loss_curve_observation}節）および初回誤分類反復数の定量分析（第\ref{sec:misclassification_analysis}節）の結果を総合的に考察する．
...
\begin{itemize}
    ...
    \item 脆弱なモデルに対しては，DeepFool初期化を用いることで反復0で誤分類を達成でき，初回誤分類反復数のみを評価する場合は有効な初期化手法である
    \item Multi-DeepFool初期化はPGDの反復数が50回未満で済む場合はランダム初期化（9回リスタート）と比較して計算コストが低くなる（付録\ref{appendix:computational_cost}参照）．条件を満たす場合，脆弱なモデルにおいて誤分類が達成される場合に初回誤分類反復数，計算コスト両面で他の手法に比べ有効であり，DeepFoolと比べて約クラス数分の多様な探索が可能な点も評価できる
