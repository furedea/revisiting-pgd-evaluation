...
\begin{enumerate}
    ...
    \item 全てのモデル（nat, adv, nat\_and\_adv, weak\_adv）が自然画像の状態で正しく分類するサンプルを抽出
    \item 最初に見つかった5枚（または1枚）を評価用サンプルとして使用
\end{enumerate}

この選択基準は，PGD攻撃の前提条件である「元々正しく分類されているサンプルに対する攻撃」を満たすためである．攻撃前から誤分類されているサンプルを評価対象に含めると，攻撃の効果を正しく評価できない．また，全てのモデルで共通のサンプルを使用することで，異なるモデル間での損失曲線や誤分類速度の比較が容易になる．
...
縦軸は試行番号（Multi-DeepFool初期化では境界までの距離の順位），横軸は反復回数を表す．各セルの色は，その時点での敵対的サンプルに対するモデルの予測結果を示す．黄色は正しく分類（攻撃失敗），紫色は誤分類（攻撃成功）を意味する．このヒートマップにより，各試行がいつ攻撃に成功したかを視覚的に確認できる．
...
\begin{figure}[htbp]
  ...
  \caption{MNIST natモデルに対するランダム初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なる試行）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，ランダム初期化サンプル，PGD敵対的サンプルの比較．}
  \label{fig:mnist_nat_random}
\end{figure}

\medskip
図\ref{fig:mnist_nat_random}より，損失曲線を見ると，20回の試行は異なるランダム初期点から開始しているが，いずれも同程度の値に収束している．これはMadryらの「異なる初期点から開始しても同程度の局所最大解に収束する」という観察と一致する．ヒートマップを見ると，全5サンプルにおいて20回の試行全てが100反復以内に誤分類（紫色）を達成している．初回誤分類反復数はサンプルにより5〜15反復程度の幅がある．
...
\begin{figure}[htbp]
  ...
  \caption{MNIST advモデルに対するランダム初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なる試行）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，ランダム初期化サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 natモデルに対するランダム初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なる試行）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，ランダム初期化サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 advモデルに対するランダム初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なる試行）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，ランダム初期化サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{MNIST natモデルに対するDeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，DeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{MNIST advモデルに対するDeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，DeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 natモデルに対するDeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，DeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 advモデルに対するDeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，DeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{MNIST natモデルに対するMulti-DeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なるターゲットラベル）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，損失が最大となるDeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{MNIST advモデルに対するMulti-DeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なるターゲットラベル）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，損失が最大となるDeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 natモデルに対するMulti-DeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なるターゲットラベル）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，損失が最大となるDeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10 advモデルに対するMulti-DeepFool初期化PGD攻撃の結果．上段：損失曲線（横軸は反復回数，縦軸は損失値，各線は異なるターゲットラベル）．中段：正誤ヒートマップ（黄色は正しい分類，紫色は誤分類）．下段：元画像，損失が最大となるDeepFool敵対的サンプル，PGD敵対的サンプルの比較．}
...
\begin{figure}[htbp]
  ...
  \caption{MNISTにおける誤分類CDF．横軸は反復回数，縦軸はその反復回数までに誤分類を達成したサンプルの累積割合．色は初期化手法を表す（青：クリーン，緑：ランダム，赤：DeepFool，オレンジ：Multi-DeepFool）．マーカーと線種はモデルを表す（丸・実線：nat，四角・破線：nat\_and\_adv，三角・一点鎖線：adv，ダイヤ・点線：weak\_adv）．攻撃に失敗した組み合わせはグレーで表示され，凡例に[FAILED]と記載．}
...
図中の色は初期化手法，線種とマーカーはモデルを表す．攻撃に失敗した組み合わせ（advモデルおよびnat\_and\_advモデル）はグレーで表示され，凡例に[FAILED]と記載されている．

\medskip
図\ref{fig:mnist_misclassification_cdf}より，natモデルとweak\_advモデルのみが100反復以内に誤分類を達成しており，advモデルとnat\_and\_advモデルはどの初期化手法を用いても攻撃に失敗している（グレーの[FAILED]）．natモデルでは，DeepFool初期化（赤丸）が反復0で100\%に達しており最も高速である．Multi-DeepFool初期化（オレンジ実線）は反復0で約33\%から開始し反復1で100\%に達している．ランダム初期化（緑実線）は反復5から上昇を開始し反復8で100\%に達している．クリーン初期化（青丸）は反復12で100\%に達している．weak\_advモデルでは，DeepFool初期化（赤ダイヤ）が反復22で100\%に達しており最も高速である．ランダム初期化（緑破線）とMulti-DeepFool初期化（オレンジ破線）は反復22〜35にかけて徐々に上昇し100\%に達している．クリーン初期化（青ダイヤ）は反復35で100\%に達しており最も遅い．
...
\begin{figure}[htbp]
  ...
  \caption{CIFAR10における誤分類CDF．横軸は反復回数，縦軸はその反復回数までに誤分類を達成したサンプルの累積割合．色は初期化手法を表す（青：クリーン，緑：ランダム，赤：DeepFool，オレンジ：Multi-DeepFool）．マーカーと線種はモデルを表す（丸・実線：nat，四角・長破線：nat\_and\_adv，三角・一点鎖線：adv，ダイヤ・点線：weak\_adv）．攻撃に失敗した組み合わせはグレーで表示され，凡例に[FAILED]と記載．同じ反復回数で誤分類を達成した曲線が重なるため，視認性のために横方向に微小なオフセットを加えている．}
  \label{fig:cifar10_misclassification_cdf}
\end{figure}

\medskip
図\ref{fig:cifar10_misclassification_cdf}より，advモデルのみが攻撃に失敗しており（グレーの[FAILED]），他の3モデル（nat, nat\_and\_adv, weak\_adv）は全て反復2以内に100\%の誤分類を達成している．MNISTと比較して桁違いに高速である．natモデルとnat\_and\_advモデルでは，DeepFool初期化（赤丸・赤四角）が反復0で100\%に達している．Multi-DeepFool初期化（オレンジ実線）は反復0で約50〜66\%から開始し反復1で100\%に達している．クリーン初期化（青丸・青四角）は反復1で100\%に達している．ランダム初期化（緑実線）は反復1で約66\%、反復2で100\%に達している．weak\_advモデルでは，Multi-DeepFool初期化（オレンジ点線）が反復0で約65\%から開始し最も高速である．DeepFool初期化（赤ダイヤ）は反復1で100\%に達している．クリーン初期化（青ダイヤ）とランダム初期化（緑点線）は反復2で100\%に達している．
...
本節では，損失曲線の観察（第\ref{sec:loss_curve_observation}節）および誤分類速度の定量分析（第\ref{sec:misclassification_analysis}節）の結果を総合的に考察する．
...
\begin{itemize}
    \item 堅牢なモデル（MNIST adv, nat\_and\_adv，CIFAR10 adv）に対しては，100反復のPGD攻撃でも誤分類を達成できない．より長い反復回数や他の攻撃手法の検討が必要である
    \item CIFAR10のような高次元データでは，誤分類が2反復以内に達成されるため，反復回数よりも攻撃の成否（攻撃成功率）が重要な指標となる
    \item 脆弱なモデルに対しては，DeepFool初期化を用いることで反復0で誤分類を達成でき，誤分類速度のみを評価する場合は有効な初期化手法である
    \item Multi-DeepFool初期化はPGDの反復回数が50回未満で済む場合はランダム初期化（9回リスタート）と比較して計算コストが低くなる（付録\ref{appendix:computational_cost}参照）．条件を満たす場合，脆弱なモデルにおいて誤分類が達成される場合に誤分類速度，計算コスト両面で他の手法に比べ有効であり，DeepFoolと比べて約クラス数分の多様な探索が可能な点も評価できる
