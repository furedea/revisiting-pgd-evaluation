...
誤分類達成反復数の定量分析では，単一サンプルに依存しない一般的な傾向を把握することが目的である．特定のラベルに偏らないサンプル集合を得るため，以下の手順で各ラベル（0〜9）から1枚ずつサンプルを選択する．
...
\begin{enumerate}
    ...
    \item 抽出されたサンプルの中から，乱数シード（$\text{seed}=0$）を固定した上で1枚をランダムに選択する
\end{enumerate}

これにより，計10枚のサンプルを得る．ランダム選択により，テストデータの並び順に依存しない代表的なサンプル集合が得られる．

誤分類達成反復数の定量分析では，各モデル・各初期化手法について以下の手順で結果を算出する．

\begin{enumerate}
    \item 10サンプルそれぞれに対してPGD攻撃をリスタート数分実行
    \item 各サンプルごとに，誤分類達成反復数の統計量（誤分類達成率，平均，中央値，P95など）および各反復時点で誤分類している試行の割合を算出
...
第\ref{sec:sample_selection_quantitative}節で述べた通り，本節の分析では各ラベルから1枚ずつ選択した計10サンプルの結果を平均して1つの代表値として扱う．

\paragraph{誤分類ヒートマップの読み方}
誤分類ヒートマップは，横軸にPGD反復数（0〜100），縦軸にモデルと初期化手法の組み合わせを示す．各セルの色は，その反復数時点で誤分類している試行の割合（10サンプル平均）を表す．緑色は誤分類達成率0\%（全試行が正しく分類），赤色は誤分類達成率100\%（全試行が誤分類）を意味する．左側から赤くなるほど少ない反復数で誤分類を達成しており，100反復でも緑色のままの行は攻撃が失敗したことを示す．
...
\begin{itemize}
    \item \textbf{nat}：DeepFool初期化およびMulti-DeepFool初期化は反復0から黄色や赤，すなわち誤分類達成率50\%以上を示し，DeepFool初期化の時点から誤分類を達成している試行が多く存在する．ランダム初期化は反復5付近，クリーン初期化は反復10から20ほどで赤(誤分類達成率100\%)に到達している．
    \item \textbf{nat\_and\_adv}：全体的に緑や黄色，すなわち誤分類達成率50\%以下が支配的である．DeepFool初期化は反復5付近から黄色に移行する一方，ランダム初期化はほぼ緑色，すなわち誤分類達成率20\%ほどのままである．
    \item \textbf{adv}：全4つの初期化手法で100反復を通して緑色，すなわち誤分類達成率0〜5\%ほどを維持しており，PGD攻撃に対して非常に堅牢であることがわかる．
    \item \textbf{weak\_adv}：反復20から50付近で緑色から赤色に移行し，natと比較して誤分類達成に多くの反復数を要する．DeepFool初期化の誤分類達成率は他の初期化手法と比較して高く，50反復ほどで赤色(誤分類達成率100\%)に到達する．
\end{itemize}
...
\begin{itemize}
    \item \textbf{nat}：全初期化手法の全試行で誤分類している（誤分類達成率100\%）．DeepFool初期化が最も高速（平均0.1反復）であり，次いでMulti-DeepFool初期化（平均0.5反復），ランダム初期化（平均5.6反復），クリーン初期化（平均10.1反復）の順である．
    \item \textbf{nat\_and\_adv}：誤分類達成率は7〜40\%と低い．DeepFool初期化が最も高い誤分類達成率（40\%）かつ最も高速（平均6.2反復）であり，他の初期化手法と比較し大幅に少ない反復数で誤分類を達成している．一方，ランダム初期化は誤分類達成率が7\%と極めて低く，誤分類達成に要する反復数も平均64.9反復と非常に多い．
    \item \textbf{adv}：クリーン，DeepFool，Multi-DeepFool初期化で誤分類達成率0\%，ランダム初期化でも2\%であり，100反復のPGD攻撃に対して非常に堅牢であることがわかる．
    \item \textbf{weak\_adv}：誤分類達成率は90〜100\%であり，natモデルに近い脆弱性を示す．DeepFool初期化が最も高速（平均20.8反復）で，ランダム初期化（平均25.2反復），Multi DeepFool初期化（平均26.2反復），クリーン初期化（平均32.3反復）の順である．
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|cccc}
    ...
    モデル & 初期化 & 誤分類達成率 & 平均 & 中央値 & P95 \\
    \hline
    \multirow{4}{*}{nat} & clean & 100\% & 1.4 & 1.4 & 1.4 \\
     & random & 100\% & 1.3 & 1.2 & 1.6 \\
     & deepfool & 100\% & 0.1 & 0.1 & 0.1 \\
     & multi\_deepfool & 100\% & 0.3 & 0.2 & 0.8 \\
    \hline
    \multirow{4}{*}{nat\_and\_adv} & clean & 60\% & 3.8 & 3.8 & 3.8 \\
     & random & 60\% & 4.4 & 4.3 & 4.9 \\
     & deepfool & 60\% & 1.7 & 1.7 & 1.7 \\
     & multi\_deepfool & 60\% & 4.0 & 3.2 & 8.9 \\
    \hline
    \multirow{4}{*}{adv} & clean & 60\% & 4.7 & 4.7 & 4.7 \\
     & random & 57\% & 5.5 & 5.5 & 6.2 \\
     & deepfool & 50\% & 2.0 & 2.0 & 2.0 \\
     & multi\_deepfool & 58\% & 4.1 & 4.2 & 5.6 \\
    \hline
    \multirow{4}{*}{weak\_adv} & clean & 50\% & 4.8 & 4.8 & 4.8 \\
     & random & 50\% & 5.6 & 5.4 & 6.4 \\
     & deepfool & 40\% & 1.8 & 1.8 & 1.8 \\
     & multi\_deepfool & 51\% & 7.1 & 7.2 & 9.7 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{itemize}
    \item ほぼ全てのモデル・初期化手法の組み合わせで40\%以上の誤分類達成率を示し，特にadvモデルはMNISTのもの（誤分類達成率0〜2\%）と比較して攻撃が容易である．
    \item \textbf{nat}：全ての初期化手法で誤分類達成率100\%を示した．DeepFool初期化が最も高速（平均0.1反復）で，Multi-DeepFool初期化（平均0.3反復），ランダム初期化（平均1.3反復），クリーン初期化（平均1.4反復）の順である．
    \item \textbf{nat\_and\_adv}：全ての初期化手法で誤分類達成率60\%を示した．DeepFool初期化が最も高速（平均1.7反復）で，クリーン初期化（平均3.8反復），Multi-DeepFool初期化（平均4.0反復），random初期化（平均4.4反復）の順である．
    \item \textbf{adv}：MNISTのadvモデル（0〜2\%）とは対照的に，全ての初期化手法で50\%以上の誤分類達成率を示した．DeepFool初期化が最も高速（平均2.0反復）で，Multi-DeepFool初期化（平均4.1反復），クリーン初期化（平均4.7反復），ランダム初期化（平均5.5反復）の順である．
    \item \textbf{weak\_adv}：全ての初期化手法で40\%以上の誤分類達成率を示し，MNISTのweak\_advモデル（90〜100\%）と比較して低い．DeepFool初期化が最も高速（平均1.8反復）で，クリーン初期化（平均4.8反復），ランダム初期化（平均5.6反復），Multi-DeepFool初期化（平均7.1反復）の順である．
\end{itemize}
...
\begin{itemize}
    \item MNISTのnatモデル：ランダム初期化で平均5.6反復，クリーン初期化で平均10.1反復
    \item CIFAR10のnatモデル：ランダム初期化で平均1.3反復，クリーン初期化で平均1.4反復
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{CIFAR10}：nat，nat\_and\_adv，adv，weak\_advの順となり，weak\_advが最も低い誤分類達成率（40--51\%）を示した．
\end{itemize}
CIFAR10でweak\_advがadvより低い誤分類達成率を示した理由として，CIFAR10の$\varepsilon = 8/255 \approx 3\%$はMNISTの$\varepsilon = 0.3$（30\%）と比較して非常に小さく，advとweak\_advの敵対的訓練の差が出にくいことが考えられる．その中で各ラベルにつき1枚ずつのサンプルで平均を取っているため，統計的なばらつきにより逆転が生じている可能性がある．

\subsection{初期化手法の効果}
初期化手法の効果は，モデルの堅牢性に強く依存する：
\begin{itemize}
    \item \textbf{脆弱なモデル（nat）}：DeepFool初期化およびMulti-DeepFool初期化により反復0〜1で誤分類を達成している．MNISTのnatモデルでは，DeepFool初期化（平均0.1反復）とMulti-DeepFool初期化（平均0.5反復）がランダム初期化（平均5.6反復）より大幅に高速である．
    \item \textbf{堅牢なモデル（MNIST adv）}：どの初期化手法を用いても100反復以内に誤分類をほぼ達成できない（誤分類達成率0--2\%）．損失曲線（図\ref{fig:mnist_adv_random}，\ref{fig:mnist_adv_deepfool}，\ref{fig:mnist_adv_mdf}）でも損失値は$10^{-5}$〜$10^{-2}$程度と非常に小さい範囲に留まる
\end{itemize}
...
\begin{itemize}
    ...
    \item Multi-DeepFool初期化は，DeepFool初期化と比べて約ラベル数分の多様な探索が可能である．
    \item CIFAR10では，モデルによらずサンプルによって攻撃の成否が異なるため，ロバスト性評価には複数サンプルでの検証が重要である．
\end{itemize}
